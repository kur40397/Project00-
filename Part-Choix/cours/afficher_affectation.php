...
<body>
    ...
    $stm = $pdo->prepare("select * from professeur,enseignement where 
    professeur.id_prof=enseignement.id_prof and enseignement.id_cours=?");
    $stm->execute([$idc]);
    ...
    if (empty($tab)) {
        ...
        echo '</thead>';
        foreach ($tab as $p) {
            echo "<tr><td>" . $p["id_prof"] . "</td><td>"
                . $p["nom"] . "</td><td>" . $p["prenom"] .
                "</td><td>" . $p["cin"] . "</td><td>" . $p["adresse"]
                . "</td><td>" . $p["telephone"] . "</td><td>" . $p["email"]
                . "</td><td>" . $p["date_recrutement"] . "</td></tr>";
        }
    }
    ?>
    </table>
    ...
    <br>
    <div class="col-12">
        <a href="../cours1.php" class="btn btn-dark">Annuler</a>
    </div>
</body>
